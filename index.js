...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀:
// nested function is accesible through myfunction due to closure
...
function animalNames(animalArr) {
  ...
  animalArr.forEach((animal) => {
    ...
    displayNames.push(`name: ${animalName}, scientific: ${scientificName}`);
  });
  return displayNames;
}
...
function lowPopulationAnimals(zooAnimals) {
  const lessThanFive = zooAnimals.filter((animal) => animal.population < 5);
  return lessThanFive;
}
...
function USApop(zooAnimals) {
  const totalPop = zooAnimals.reduce((acc, animal) => {
